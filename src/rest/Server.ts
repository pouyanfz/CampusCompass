...
dotenv.config({ path: path.resolve(process.cwd(), "src/rest/.env.local") });
...
export default class Server {
  ...
  private handlePutDataset = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    try {
      ...
      const result = await this.insightFacade.addDataset(
        id,
        base64,
  ...
  private handleDeleteDataset = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    try {
  ...
  private handleGetDatasets = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    try {
  ...
  private handleGetDistance = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    try {
      ...
      if (!origin || !destination) {
        res
          ...
          .json({ error: "Missing origin or destination" });
        return;
      }
