...
class QueryValidator {
    ...
    static isValidMComparator(obj, kind) {
        ...
        const key = keys[0];
        const val = obj[key];
        const expectedFields = kind === IInsightFacade_1.InsightDatasetKind.Sections ? this.sectionsFields.mfields : this.roomsFields.mfields;
    ...
    static isValidSComparator(obj, kind) {
        ...
        const key = keys[0];
        const val = obj[key];
        const expectedFields = kind === IInsightFacade_1.InsightDatasetKind.Sections ? this.sectionsFields.sfields : this.roomsFields.sfields;
        if (!expectedFields.includes(key.split("_")[1]))
            throw new IInsightFacade_1.InsightError("Invalid key " + key);
